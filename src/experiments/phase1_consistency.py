"""
Phase I: Moral Decision Consistency Experiment Runner.

Tests whether LLMs make the same moral decision when presented with
identical dilemmas multiple times under various temperature settings.
"""
...
class Phase1Runner:
    """Runs Phase I consistency experiments."""
    ...
    def __init__(
        ...
        """
        Initialize Phase I experiment runner.
        ...
        if config.experiment_type not in ["phase1_consistency", "pilot"]:
            raise ValueError(
                f"Invalid experiment type for Phase1Runner: {config.experiment_type}"
            )
    ...
    def run_experiment(self) -> str:
        ...
        if self.config.test_reversed_order:
            total_runs *= 2  # Double for reversed order
            ...
            for model_name in self.config.models:
                ...
                for dilemma_id in dilemma_order:
                    ...
                    for temperature in self.config.temperatures:
                        # Determine position orders to test
                        position_orders = ["original"]
                        if self.config.test_reversed_order:
                            position_orders.append("reversed")
...
def run_phase1_experiment(experiment_name: str = "phase1_consistency") -> str:
    """
    Convenience function to run a Phase I experiment from config.

    Args:
        experiment_name: Name of experiment in config (default: "phase1_consistency")
